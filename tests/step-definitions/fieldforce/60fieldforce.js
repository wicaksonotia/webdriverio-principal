...
When(/^I input form and submit$/, async () => {
    await FormFieldForcePage.createFF(dataTest.fieldForce.name, dataTest.fieldForce.username)
    await FormFieldForcePage.submitFF()
});

Then(/^System should be able to submit form Field Force and leading to Field Force page$/, async () => {
    await FieldForcePage.page()
    await FieldForcePage.pageList()
    await FieldForcePage.searchFF(dataTest.fieldForce.name)
    await FieldForcePage.expectNewFF(dataTest.fieldForce.name, dataTest.fieldForce.classification, dataTest.fieldForce.status)
});
...
When(/^I edit and delete Field Force$/, async () => {
    ...
    await FieldForcePage.pageList()
    await FieldForcePage.searchFF(dataTest.fieldForce.name)
    await FieldForcePage.expectNewFFEdited(dataTest.fieldForce.name, dataTest.fieldForce.classificationEdit, dataTest.fieldForce.statusEdit)
