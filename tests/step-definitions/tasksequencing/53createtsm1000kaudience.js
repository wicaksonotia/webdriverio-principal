...
When(/^I create task sequencing and click button Submit$/, async () => {
    ...
    await FormCreatePage.createTSM(dataTest.taskSequencing.nameWithPinUp53, dataTest.tradeProgram.namaTP4, dataTest.audience.audience53)
...
When(/^setting Misi and save mission builder$/, async () => {
    ...
    await MissionBuilderPage.settingMissionWithoutPinUp(dataTest.templateTugas.searchByName42)
    await MissionBuilderPage.setVerifikasiManual()
    await MissionBuilderPage.setCoin(dataTest.taskSequencing.coinSubmission53, dataTest.taskSequencing.coinVerification53)
