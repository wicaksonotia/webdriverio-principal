...
When(/^I create task sequencing and click button Submit$/, async () => {
    ...
    await FormCreatePage.createTSM(dataTest.taskSequencing.nameWithoutPinUp5, dataTest.tradeProgram.namaTP4, dataTest.audience.audienceToko)
...
When(/^setting Misi without Pin Up Mission$/, async () => {
    ...
    await MissionBuilderPage.settingMissionWithoutPinUp(dataTest.templateTugas.searchByName2)  //Mission 2
    await MissionBuilderPage.setVerifikasiFF()
    await MissionBuilderPage.setCoin(dataTest.taskSequencing.coinSubmission5, dataTest.taskSequencing.coinVerification5)
...
Then(/^System should be leading to Task Sequencing page and success save new Tsm$/, async () => {
    ...
    // await TaskSequencingPage.getAlert()
});
...
When(/^I view detail task$/, async () => {
    await TaskSequencingPage.detailTask(dataTest.taskSequencing.nameWithoutPinUp5)
    await FormCreatePage.page()
