...
allureReporter.addTestId('[PRINCIPAL] 27. Task Sequencing - Request Approval Coin Adjustment');
...
When(/^I search a task sequencing for import$/, async () => {
    await TaskSequencingPage.searchTaskSequencing(dataTest.taskSequencing.nameWithoutPinUp5)
});

Then(/^System should be according data task sequencing based on search keyword$/, async () => {
    await TaskSequencingPage.searchResult(dataTest.taskSequencing.nameWithoutPinUp5)
});
...
When(/^I import coin and confirm$/, async () => {
    ...
    await FormCreatePage.userPenerimaCoinAdj(dataTest.taskSequencing.reasonImportCoin, dataTest.login.accountname)
