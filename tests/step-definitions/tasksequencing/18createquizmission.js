...
When(/^I search a template tugas$/, async () => {
    await TemplateTugasPage.searchTemplateTugas(dataTest.templateTugas.searchByName18)
});
...
Then(/^System should be show detail template tugas page with type quiz$/, async () => {
...
When(/^I create task sequencing and click button Submit$/, async () => {
    ...
    await FormCreatePage.createTSM(dataTest.taskSequencing.nameTask18, dataTest.tradeProgram.namaTP18, dataTest.audience.audienceToko)
...
When(/^setting Misi without Pin Up Mission$/, async () => {
    ...
    await MissionBuilderPage.settingMissionWithoutPinUp(dataTest.templateTugas.searchByName18)
    await MissionBuilderPage.setVerifikasiManual()
    await MissionBuilderPage.setCoinSubmission(dataTest.taskSequencing.coinSubmission18)
