...
When(/^I search a task sequencing for import$/, async () => {
    await TaskSequencingPage.searchTaskSequencing(dataTest.taskSequencing.nameWithPinUp4) 
});

Then(/^System should be according data task sequencing based on search keyword$/, async () => {
    await TaskSequencingPage.searchResult(dataTest.taskSequencing.nameWithPinUp4)
});
