...
Then(/^System should be able to submit Check Image and leading to Check Image page$/, async () => {
    await CheckImagePage.pageList()
    await CheckImagePage.expectNewCheckImageRow1(dataTest.IR.templateNameCheckImage, dataTest.IR.statusReq)
...
When(/^status is changed from requesting to done$/, async () => {
    ...
    await browser.pause(10000)
    await CheckImagePage.page()
    await CheckImagePage.expectNewCheckImageRow1(dataTest.IR.templateNameCheckImage, dataTest.IR.statusDone)
