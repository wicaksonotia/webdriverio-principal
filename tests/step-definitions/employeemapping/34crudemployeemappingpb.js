...
When(/^I input form with metode pembayaran 'Pojok Bayar' and submit$/, async () => {
    await FormEmployeeMappingPage.name(dataTest.employeeMapping.searchByNamaEmployee1)
    await browser.pause(2000)
    await FormEmployeeMappingPage.metodePB()
    await browser.pause(2000)
...
Then(/^System should be display employee data based on search keyword$/, async () => {
    await EmployeeMappingPage.expectNewEmployeeRow1(dataTest.employeeMapping.employee, dataTest.employeeMapping.email, dataTest.employeeMapping.area, dataTest.employeeMapping.metodePB)
});
