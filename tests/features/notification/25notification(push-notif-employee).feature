Feature: 25. Push Notifikasi (push-notif-employee-only)

  Scenario Outline: As a user, I can Create Push Notifikasi Employee Only
    ...
    When I select Employee only in Filter Pengguna Tipe Employee
    ...
    Examples:
      ...
      | push notif employee only Testing Reda | push notif employee only Testing Reda | push notif employee only Testing Reda | push notif employee only Testing Reda |
