...
class FormAudiencePage extends Page {
    ...
    get titlePage () { return $('#headerAudience') }
    get titlePagePersonalize () { return $('#headerAudiencePersonalize') }

    get namaGrup () { return $('input[formcontrolname="name"]') }
    get publishMisi () { return $('[formcontrolname="mission_publication_id"]') }
    get jenis () { return $$('mat-radio-group[formcontrolname="audience_type"] mat-radio-button') } //Tsm, Scheduler
    get businessType () { return $('[formcontrolname="business_type"]') }
    get limit () { return $$('mat-radio-group[formcontrolname="limit"] mat-radio-button') } //Batasi Audience, Pilih Semua
    get min () { return $('input[formcontrolname="min"]') }
    get max () { return $('input[formcontrolname="max"]') }
    get type () { return $$('mat-radio-group[formcontrolname="type"] mat-radio-button') }
    get dropdownTradeScheduler () { return $('[formcontrolname="trade_scheduler_id"]') }
    get searchTradeScheduler () { return $('ngx-mat-select-search div input') }
    get selectTradeScheduler () { return $$('[role="option"]') }
    get rbFixedPanel () { return $$('[class="mat-radio-container"]') }

    get InputTextSearch () {return $$('[placeholder="Cari"]')}
    ...
    get filterRetailClassification () { return $('') }
    get filterSrcClassification () { return $('') }
    get filterSrcType () { return $('') }
    get btnLoaddata () { return $('div.pt-16.ng-star-inserted > button') }
    get btnExportImport () { return $$('[class="is-danger mat-raised-button"]') }
    get btnCheck () { return $('#btn-check') }
    
    get btnImport () { return $('div.p-0 > div > div:nth-child(1) > button') }
    get btnImportAfterClose () { return $('div:nth-child(1) > div:nth-child(3) > div > button') }
    get textRequestingImport () { return $('div.p-0 > div > div:nth-child(1) > label') }
    get popUpDialogImport () { return $('[role="dialog"] ng-component div  mat-toolbar div div span') }
    get inputAddFile () { return $('div.mat-dialog-content.ps > div > label input[type="file"]') }
    get progressImport () { return $('[role="progressbar"]') }
    get datatableRowImport () { return $$('[role="dialog"] datatable-scroller > datatable-row-wrapper') }
    get cellTableImport () { return $$('[role="dialog"] datatable-scroller > datatable-row-wrapper > datatable-body-row > div > datatable-body-cell > div') }
    get btnCloseDialogImport () { return $('[role="dialog"] button.mat-icon-button') }
    get btnKirimDialogImport () { return $('[role="dialog"] button.mat-raised-button') }
    get btnBatalDialogImport () { return $('[role="dialog"] button.mat-stroked-button') }

    get btnExport () { return $('div.p-0 > div > div:nth-child(2) > button') }
    get btnSimpan () { return $('#btn-simpan') }
    ...
    async pageCreatePersonalize () {
        ...
        await expect(this.titlePagePersonalize).toHaveText('Buat Audience Personalize')
        await expect(browser).toHaveUrlContaining('dte/audience/create-personalize')
    }
    ...
    async createImportPersonalize (namaGrup) {
        ...
        await this.InputTextSearch[1].addValue('Misi perso 0203 recommended panel')
    ...
    async importFile () {
        ...
        await browser.execute(function () {
            document.querySelector('div.mat-dialog-content.ps > div > label').style="visibility:visible; overflow:visible;";
          });
        await browser.pause(5000);
